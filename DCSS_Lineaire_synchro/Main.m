...
B = 125e6; %Largeur de bande
% SF = [9,8,7]; %Nombre de bits par symbole
SF = 7;
...
eb=1;
eb_n0_dB = -10:-5; %Rapport signal su bruit
% eb_n0_dB = 100;
...
for k=1:length(SF)
    for i = 1:length(eb_n0)
        ...
        while bit_cnt < 1e6
            ...
            %% Demodulatio
            y = Dechirp(yc,SF(k),B,alpha,1);

            Dnums_est = DemodChirp(y,SF(k));
            nums_est = DeDiffp(Dnums_est,SF(k));
